  ...
  useEffect(() => {
    async function fetchApi() {
      ...
      if (data.message === 'Has error') {
        ...
        setType(data.hotel.type)
        setCity(data.hotel.city)
        setAddress(data.hotel.address)
        setDistance(data.hotel.distance)
        setTitle(data.hotel.title)
        setPhotos(data.hotel.photos.toString())
        setDesc(data.hotel.desc)
        setFeatured(data.hotel.featured)
        setRooms(data.hotel.rooms)
        setCheapestPrice(data.hotel.cheapestPrice)
      }
    }
    fetchApi()
  }, [])
      ...
      <div>
        ...
        <div style={{ boxShadow: ' rgba(0, 0, 0, 0.24) 0px 3px 8px', margin: '18px 10px 8px 10px', padding: '8px 40px' }}>
          <div className='wrapper-input'>
            {input('Name', 'text', setName, 'Name of the hotel', name)}
            <div>
              <label >Type</label><br />
            ...
            {input('City', 'text', setCity, 'City of the hotel', city)}
            {input('Address', 'text', setAddress, 'Address of the hotel', address)}
            {input('Distance from city center', 'number', setDistance, 'Distance from city center', distance)}
            {input('Title', 'text', setTitle, 'Title of the hotel', title)}
            {input('Description', 'text', setDesc, 'Description', desc)}
            {input('Price', 'number', setCheapestPrice, 'Cheapest price', cheapestPrice)}
            {input('Images', 'text', setPhotos, 'Give comma between link image', photos)}
            <div>
